...
(function() {
    ...
    function reindex() {
        tbody.querySelectorAll('.item-row').forEach((row, i) => { row.querySelector('.row-number').textContent = i + 1; });
    }
    ...
    function hitungTotal() {
        let total = 0;
        document.querySelectorAll('.jumlah-input').forEach(function(input) { total += parseInt(input.value.replace(/\./g, '')) || 0; });
    ...
    function initSelect2New(container) {
        if (typeof $ !== 'undefined' && $.fn.select2) {
            $(container).find('.select2-perkiraan').select2({ theme: 'bootstrap-5', placeholder: '-- Pilih --', allowClear: true, width: '100%' });
        }
    }
...
function fillNamaPerkiraan(selectEl) {
    const row = selectEl.closest('tr') || selectEl.closest('.row') || selectEl.closest('.card-body');
    const namaInput = row ? row.querySelector('.nama-perkiraan-input') : null;
    if (!namaInput) return;
